...
if __name__ == '__main__':
    ...
    for fn in tqdm(fns):
        ...
        try:
            ...
            for i, j in itertools.product(range(patches_img.shape[0]), range(patches_img.shape[1])):
                single_patch_img = patches_img[i,j,:,:]
                single_patch_img = single_patch_img[0]
                cv2.imwrite(PATCH_DIR+'/images/'+fn.name.split('.')[0]+f'_{i}_{j}.png', single_patch_img)
                count += 1
        except Exception:
            print(fn.name)
    ...
    for fn in tqdm(fns):
        ...
        try:
            ...
            for i, j in itertools.product(range(patches_img.shape[0]), range(patches_img.shape[1])):
                single_patch_img = patches_img[i,j,:,:]
                single_patch_img = single_patch_img[0]
                cv2.imwrite(PATCH_DIR+'/labels/'+fn.name.split('.')[0]+f'_{i}_{j}.png', single_patch_img)
        except Exception:
            print(fn.name)
